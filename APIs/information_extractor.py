...
class Predictor():
    ...
    def load_text_detect_model(self, gpu=False):
        self.model_text_detect = PaddleOCR(use_gpu=(True if torch.cuda.is_available() else False))
        ...
        for index, row in locate.iterrows():
            ...
            if float(prob) < 0.7:
                ...
                for box in boxes:
                    ...
                    xmin, ymin = min(top_left[0], bottom_right[0]), min(
                        top_left[1], bottom_right[1])
                    xmax, ymax = max(top_left[0], bottom_right[0]), max(
                        top_left[1], bottom_right[1])
                    ymax = int(ymax+10)
                    # ymin=int(ymin*1.2)
                    ...
                    name_img = "crop/" + \
                        str(row["class"])+"/"+str(index)+"_"+str(count)+".jpg"
