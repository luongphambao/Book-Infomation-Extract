...
from preprocess.pyimagesearch import transform
from preprocess.pyimagesearch import imutils
...
class DocScanner(object):
    ...
    def scan(self, img_path, output_path=None, binary=False):
        print(img_path)
        ...
        if output_path is not None:
            ...
            print(output_path)
            cv2.imwrite(output_path+"/"+img_path.split("/")[-1], warped)
            #cv2.imwrite(output_path, warped)
        else:
            warped = cv2.cvtColor(warped, cv2.COLOR_BGR2RGB)
            return warped

def preprocess_background(img_path, output_path=None, binary=False):
    scanner = DocScanner()
    #print(img_path)
    scanner.scan(img_path, output_path, binary)
